/**
 * TaskOverlay.jsx
 * Floating guided-task HUD rendered fixed on screen.
 * Shows: lesson title, progress bar, current instruction, step counter,
 *        Skip / Stop / Next buttons.
 *
    ...
    >
      ...
      {minimized && (
        <button onClick={() => setMinimized(false)}
          style={{ width: 52, height: 52, background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}
      ...
      {!minimized && (
        <>
          ...
          <div style={{
            ...
            <span style={{ fontSize: 13, fontWeight: 700, color: '#00b5b5', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              📋 {activeLesson.lesson_title}
            </span>
            <button onClick={() => setMinimized(true)}
              style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.4)', cursor: 'pointer', fontSize: 16, lineHeight: 1, padding: '0 2px' }}
            ...
            <button onClick={stopLesson}
              ...
              title="Exit lesson">✕</button>
          </div>

          {/* Progress bar */}
          <div style={{ height: 3, background: 'rgba(255,255,255,0.08)' }}>
            <div style={{ height: '100%', width: `${progress}%`, background: '#00b5b5', transition: 'width 0.4s' }} />
          </div>
          ...
          {isDone ? (
            <div style={{ padding: '20px 16px', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 600, color: '#fff', marginBottom: 4 }}>Lesson Complete!</div>
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', marginBottom: 14 }}>
                You finished <em>{activeLesson.lesson_title}</em>
              </div>
              <button onClick={stopLesson}
                style={{ padding: '7px 20px', background: '#00b5b5', border: 'none', borderRadius: 6, color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                Back to Dashboard
              </button>
            </div>
          ) : step ? (
            ...
            <div style={{ padding: '12px 14px 14px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                <span style={{
                  ...
                  letterSpacing: '0.06em', color: '#00b5b5',
                  background: 'rgba(0,181,181,0.12)',
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 14, justifyContent: 'flex-end' }}>
                <button onClick={skipStep}
                  style={{ padding: '5px 14px', background: 'transparent', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 6, color: 'rgba(255,255,255,0.5)', fontSize: 12, cursor: 'pointer' }}>
                  Skip
                </button>
                <button
                  onClick={() => useTaskEngine.getState().completeStep()}
                  style={{ padding: '5px 14px', background: '#00b5b5', border: 'none', borderRadius: 6, color: '#fff', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                  Mark Done →
                </button>
              </div>
            </div>
